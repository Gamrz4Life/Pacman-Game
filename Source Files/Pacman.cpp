...
void Pacman::changeColor(std::string Color)
{
	color = Color;
}
...
void Pacman::draw(bool i_victory, sf::RenderWindow& i_window)
{
	...
	if (1 == dead || 1 == i_victory)
	{
		if (animation_timer < PACMAN_DEATH_FRAMES * PACMAN_ANIMATION_SPEED)
		{
			...
			texture.loadFromFile("Resources\\Images\\PacmanDeath" + std::to_string(CELL_SIZE) + color +".png");
	...
	{
		texture.loadFromFile("Resources\\Images\\Pacman" + std::to_string(CELL_SIZE) + color + ".png");
...
void Pacman::changespeed(float multiplier) {
	PACMAN_SPEED = 2 * multiplier;
}
