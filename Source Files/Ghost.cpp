...
void Ghost::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, Ghost& i_ghost_0, Pacman& i_pacman)
{
	...
	//prevent Ghost from escaping the grid
	if (2 == frightened_mode && 0 == position.x % (unsigned char)GHOST_ESCAPE_SPEED && 0 == position.y % (unsigned char)GHOST_ESCAPE_SPEED)
	{
	...
	{
		//Frightened Ghosts get a random Direction to Run in
...
//Update Ghost Speed
void Ghost::changespeed(float multiplier)
{
	GHOST_SPEED = 2 * multiplier;
	GHOST_FRIGHTENED_SPEED = 3 * multiplier;
	GHOST_ESCAPE_SPEED = 4 * multiplier;
}
